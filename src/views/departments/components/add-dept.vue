  ...
  <el-dialog :title="showTitle" :visible="showDialog">
    ...
    <el-form label-width="120px" :model="formData" :rules="rules" ref="deptForm">
      ...
      <el-form-item label="部门介绍" prop="introduce">
        <el-input v-model="formData.introduce" style="width:80%" placeholder="1-300个字符" t
        ype="textarea" :rows="3" />
      </el-form-item>
    </el-form>
...
props: {
  ...
  data(){
    ...
    const checkNameRepeat = async(rule, value, callback) => {
      ...
      let isRepeat =false
      if(this.formData.id){
        isRepeat = depts.filter(item => item.id !== this.formData.id && item.pid === this.treeNode.pid).some
